...
A pure VBA PDF text extraction library. No external dependencies, no DLLs, no COM automation. Single `.bas` file that can be imported.
...
The module is a single `.bas` file with 21 functions. The public surface is just two; everything else is internal.
...
PDF_CleanText              strips control characters from a string (utility)
